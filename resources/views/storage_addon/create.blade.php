{{ Form::open(['route' => 'storage-addons.store', 'method' => 'post', 'id' => 'create_storage_addon_form']) }}
<div class="modal-body">
    <div class="row">
        ...
        <div class="form-group col-md-12">
            ...
            <select name="addon_type" id="addon_type" class="form-control" required onchange="toggleDurationFields(this.value)">
                <option value="plan_expiry">{{ __('Co-terminus with Company Active Plan Expiry Date') }}</option>
                <option value="fixed_duration">{{ __('Fixed Duration (Separate Validity Period)') }}</option>
            </select>
        </div>

        <div id="fixed_duration_container" class="row col-md-12 pe-0" style="display: none;">
            <div class="form-group col-md-6">
                {{ Form::label('duration_value', __('Duration Value'), ['class' => 'form-label']) }}
                {{ Form::number('duration_value', 1, ['class' => 'form-control', 'min' => '1', 'placeholder' => __('e.g. 1')]) }}
            </div>
            <div class="form-group col-md-6 pe-0">
                {{ Form::label('duration_type', __('Duration Unit'), ['class' => 'form-label']) }}
                {{ Form::select('duration_type', ['year' => __('Year(s)'), 'month' => __('Month(s)'), 'day' => __('Day(s)')], 'year', ['class' => 'form-control']) }}
            </div>
        </div>
        ...
        <div class="form-group col-md-12">
            <div class="form-check form-switch mt-2">
                ...
                <label class="form-check-label" for="is_active">{{ __('Active & Enabled') }}</label>
            </div>
        </div>
    </div>
</div>
...
<div class="modal-footer">
    ...
    <input type="submit" value="{{ __('Create Addon') }}" class="btn btn-primary">
</div>
{{ Form::close() }}

<script>
    function toggleDurationFields(val) {
        var container = document.getElementById('fixed_duration_container');
