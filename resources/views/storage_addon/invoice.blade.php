...
<head>
    ...
    <title>{{ __('Invoice') }} #INV-STRG-{{ $order->id }} - {{ $company_name }}</title>
...
<div class="modal-body p-0 invoice-container">
    ...
    <div class="d-flex align-items-center justify-content-between px-4 py-3 bg-light border-bottom no-print">
        <div class="d-flex align-items-center gap-2">
            <span class="fw-bold text-dark"><i class="ti ti-receipt me-1 text-primary fs-4"></i> {{ __('Invoice') }} #INV-STRG-{{ $order->id }}</span>
        </div>
        <div class="d-flex align-items-center gap-2">
            @if(($order->payment_type == 'Bank Transfer' || $order->payment_type == 'Manual') && !empty($order->receipt))
                <a href="{{ asset($order->receipt) }}" target="_blank" class="btn btn-sm btn-outline-secondary">
                    <i class="ti ti-paperclip me-1"></i> {{ __('View Payment Slip') }}
                </a>
            @endif
            <a href="{{ route('storage-addons.order.invoice', $order->id) }}?print=1" target="_blank" class="btn btn-sm btn-primary shadow-sm" onclick="printStorageInvoice(event, this)">
            ...
            <div class="invoice-header-box d-flex flex-wrap align-items-center justify-content-between gap-3">
                ...
                <div class="text-end">
                    ...
                    <div class="mb-2">
                        ...
                        @else
                            <span class="badge bg-danger text-white invoice-status-pill shadow-sm"><i class="ti ti-circle-x me-1"></i> {{ __('REJECTED') }}</span>
                        @endif
                    </div>
                    <div class="text-muted small">{{ __('Invoice No:') }} <strong class="text-dark">INV-STRG-{{ $order->id }}</strong></div>
                    <div class="text-muted small">{{ __('Date:') }} <strong class="text-dark">{{ $order->start_date_formatted }}</strong></div>
                </div>
            </div>
            ...
            <div class="card-body p-4">
                ...
                <div class="row mb-4 pb-3 border-bottom">
                    ...
                    <div class="col-md-6 text-md-end">
                        ...
                        <h6 class="fw-bold mb-1 text-dark fs-6">{{ $order->user ? $order->user->name : __('Company Customer') }}</h6>
                        @if($order->user && !empty($order->user->email))
                            <div class="text-muted small mb-1"><i class="ti ti-mail me-1 text-primary"></i> {{ $order->user->email }}</div>
                        @endif
                        @if($order->user && !empty($order->user->phone))
                            <div class="text-muted small mb-1"><i class="ti ti-phone me-1 text-primary"></i> {{ $order->user->phone }}</div>
                        @endif
                        @if($order->user && !empty($order->user->address))
                            <div class="text-muted small"><i class="ti ti-map-pin me-1 text-primary"></i> {{ $order->user->address }}</div>
                        @endif
                    </div>
                </div>
                ...
                <div class="table-responsive mb-4">
                    <table class="table table-bordered invoice-table align-middle">
                        <thead>
                            <tr>
                                <th>{{ __('Description / Addon Item') }}</th>
                                <th class="text-center">{{ __('Storage Allocated') }}</th>
                                <th class="text-center">{{ __('Payment Method') }}</th>
                                <th class="text-center">{{ __('Validity / Expiry') }}</th>
                        ...
                        <tbody>
                            <tr>
                                <td>
                                    <span class="fw-bold text-dark d-block fs-6 mb-1">{{ $order->addon_title }}</span>
                                    <span class="text-muted small font-monospace"><i class="ti ti-hash me-1"></i>{{ $order->order_id }}</span>
                                </td>
                                <td class="text-center">
                                    <span class="badge bg-info text-white px-3 py-2 rounded-pill fs-7 fw-semibold">{{ $order->formatted_storage }}</span>
                                </td>
                                <td class="text-center">
                                    <span class="fw-semibold text-secondary">{{ $order->payment_type }}</span>
                                </td>
                                <td class="text-center text-sm">
                                    <i class="ti ti-calendar-due me-1 text-danger"></i>{{ $order->expire_date_formatted }}
                                </td>
                ...
                <div class="row align-items-center">
                    <div class="col-md-7 mb-3 mb-md-0">
                        <div class="p-3 bg-light rounded border text-muted small">
                            ...
                            <p class="mb-0">{{ __('Thank you for your business. Additional storage space is allocated to your company storage limit upon payment approval.') }}</p>
                        </div>
                    </div>
                    <div class="col-md-5">
...
<script type="text/javascript">
    function printStorageInvoice(e, el) {
        if (e && e.preventDefault) {
            e.preventDefault();
        }
        var printUrl = "{{ route('storage-addons.order.invoice', $order->id) }}?print=1";
