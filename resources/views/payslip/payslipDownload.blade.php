...
.hdr td { border:none; padding:0 4px; vertical-align:middle; }
.hdr img { height:38px; }
.hdr-right { text-align:right; font-size:8.5px; color:#555; line-height:1.7; }

/* ── slip card ── */
.slip { border:2px solid {{ $_themeColor }}; }

/* ── slip header ── */
.slip-hdr { background:{{ $_themeColor }}12; border-bottom:2px solid {{ $_themeColor }}; }
.slip-hdr-tbl { width:100%; border-collapse:collapse; }
.slip-hdr-tbl td { border:none; padding:8px 12px; vertical-align:middle; }
.slip-hdr-right { text-align:right; font-size:8.5px; color:#444; line-height:1.7; }
.slip-hdr-tbl img { height:36px; }

/* ── title ── */
.slip-title { background:{{ $_themeColor }}; color:#fff; text-align:center; font-size:13px; font-weight:bold; letter-spacing:2px; padding:7px 0; }

/* ── band ── */
.band { background:{{ $_themeColor }}; color:#fff; font-weight:bold; font-size:9px; text-align:center; padding:4px 8px; }

/* ── base table ── */
.bt { width:100%; border-collapse:collapse; }
.bt td, .bt th { border:1px solid {{ $_themeColor }}55; padding:4px 7px; vertical-align:middle; font-size:9px; }
.bt th { background:{{ $_themeColor }}22; color:{{ $_themeColor }}; font-weight:bold; text-align:center; font-size:8.5px; }
.bt tbody tr:nth-child(even) td { background:{{ $_themeColor }}08; }
.lbl { font-weight:bold; color:#222; white-space:nowrap; }
.val { color:#444; }
.ra  { text-align:right; padding-right:7px; }

/* colour helpers */
.cg { color:#198754; font-weight:bold; }
.co { color:#e07900; font-weight:bold; }
.cr { color:#cc0000; font-weight:bold; }
.cb { color:#0056b3; font-weight:bold; }
.ct { color:{{ $_themeColor }}; font-weight:bold; }

/* totals */
.tot td { background:{{ $_themeColor }}25; font-weight:bold; color:{{ $_themeColor }}; border-top:1.5px solid {{ $_themeColor }}; }

/* net pay */
.net-lbl { background:{{ $_themeColor }}25; font-weight:bold; font-size:11px; color:{{ $_themeColor }}; text-align:center; border:1px solid {{ $_themeColor }}55; padding:10px 8px; }
.net-val { font-weight:bold; font-size:14px; color:{{ $_themeColor }}; text-align:center; border:1px solid {{ $_themeColor }}55; padding:10px 8px; }

/* stamp */
.stamp { width: 82px; height: 82px; margin: 0 auto; border: 3px solid {{ $_themeColor }}; border-radius: 50%; box-sizing: border-box; text-align: center; overflow: hidden; padding-top: 22px;   font-size: 7px; font-weight: bold; line-height: 1.4; color: {{ $_themeColor }}; }
.stamp .label { font-size: 6px; margin-top: 3px; letter-spacing: 1px; }
.stamp-img { display:block; width:80px; height:80px; border-radius:50%; object-fit:contain; border:3px solid {{ $_themeColor }}44; margin:0 auto; }
...
/* signature */
.sig { width:100%; border-collapse:collapse; margin-top:10px; }
.sig td { border:none; padding:4px 10px; vertical-align:bottom; text-align:center; }
.sig-line { border-top:1px solid #888; width:110px; margin:16px auto 3px; }

/* footer */
.foot { background:{{ $_themeColor }}; }
.foot-tbl { width:100%; border-collapse:collapse; }
.foot-tbl td { border:none; padding:5px 10px; color:#fff; font-size:8.5px; }
</style>
</head>
<body>
<div class="page">
<div class="slip">
        ...
        <table class="slip-hdr-tbl">
            <tr>
                ...
                <td class="slip-hdr-right" style="width:60%;">
                    <strong style="font-size:10.5px; color:{{ $_themeColor }};">{{ $companyName }}</strong><br>
    ...
    <tr>
        <td style="width:50%; padding:0; border:none; vertical-align:top;">
            ...
            <table class="bt">
                <tr><td class="lbl" style="width:45%;">Name</td><td class="val">{{ $employee->name }}</td></tr>
                <tr><td class="lbl">Designation</td><td class="val">{{ optional($employee->designation)->name ?? '—' }}</td></tr>
                <tr><td class="lbl">Employee ID</td><td class="val">{{ $employee->employee_id ?? '—' }}</td></tr>
                <tr><td class="lbl">Department</td><td class="val">{{ optional($employee->department)->name ?? '—' }}</td></tr>
                <tr><td class="lbl">PAN No.</td><td class="val">{{ $employee->tax_payer_id ?? '—' }}</td></tr>
                <tr><td class="lbl">Date of Joining</td><td class="val">{{ $employee->company_doj ?? '—' }}</td></tr>
            </table>
        </td>
        <td style="width:50%; padding:0; border-left:1px solid {{ $_themeColor }}55; vertical-align:top;">
            <div class="band">Payment Details</div>
            <table class="bt">
                <tr><td class="lbl" style="width:45%;">Bank Name</td><td class="val">{{ $employee->bank_name ?? '—' }}</td></tr>
                <tr><td class="lbl">Account No.</td><td class="val">{{ $employee->account_number ?? '—' }}</td></tr>
                <tr><td class="lbl">{{ \App\Models\Utility::bankCodeLabel() }}</td><td class="val">{{ $employee->bank_identifier_code ?? '—' }}</td></tr>
                <tr><td class="lbl">Account Holder</td><td class="val">{{ $employee->account_holder_name ?? '—' }}</td></tr>
                <tr><td class="lbl">Transaction</td><td class="val">NEFT</td></tr>
                <tr><td class="lbl">Pay Period</td><td class="val">{{ $slipLabel }}</td></tr>
            </table>
        </td>
    </tr>
    </table>
    ...
    <tr>
        ...
        <td style="width:34%; padding:0; border:none; vertical-align:top;">
            ...
            <table class="bt">
                <tr><th style="width:50%;">This Month</th><th>Overall</th></tr>
                <tr>
        ...
        <td style="width:28%; padding:0; border-left:1px solid {{ $_themeColor }}55; border-right:1px solid {{ $_themeColor }}55; vertical-align:top;">
            <div class="band">Days &amp; Work Details</div>
            <table class="bt">
                <tr><td class="lbl" style="width:58%;">Working Days</td><td class="ra ct">{{ $officeDays }}</td></tr>
                <tr><td class="lbl">Present Days</td><td class="ra cg">{{ $presentDaysFmt }}</td></tr>
                <tr><td class="lbl">Leave Days</td><td class="ra co">{{ $approvedLeaves }}</td></tr>
                <tr><td class="lbl">Absent Days</td><td class="ra cr">{{ $absentDaysFmt }}</td></tr>
                @if($extraDays > 0)
                <tr><td class="lbl" style="color:#856404;">Extra / OT Days</td><td class="ra" style="color:#856404; font-weight:bold;">+{{ $extraDaysFmt }}</td></tr>
                @endif
                <tr><td class="lbl">Total Hrs Worked</td><td class="ra val">{{ $totalWorkHours }} h</td></tr>
                <tr><td class="lbl">Avg Hrs / Day</td><td class="ra val">{{ $avgHrsPerDay }} h</td></tr>
            </table>
        </td>
        @endif
        ...
        <td style="width:{{ $isHourly ? '100%' : '38%' }}; padding:0; vertical-align:top; border:none;">
            ...
            <table class="bt">
                <tr><td class="lbl" style="width:50%;">Payslip Type</td><td class="ra val" style="width:50%;">{{ $payslipTypeName }}</td></tr>

                @if($isHourly)
                {{-- ── HOURLY: clean rate × hours calculation ── --}}
                <tr><td class="lbl" style="width:50%;">Hourly Rate ( <b class="ct">A</b> )</td><td class="ra ct" style="width:50%;">{{ $_fmtMoney($perHourSalary) }}</td></tr>
                <tr><td class="lbl" style="width:50%;">Hours Worked ( <b class="cg">B</b> )</td><td class="ra cg" style="width:50%;">{{ $totalWorkHours }} hrs</td></tr>
                <tr style="background:{{ $_themeColor }}10;">
                    <td class="lbl" style="width:50%;"><strong>Gross Earned</strong> ( <b class="ct">A</b> &times; <b class="ct">B</b> )</td>
                    <td class="ra ct" style="width:50%;"><strong>{{ $_fmtMoney($_storedSalary) }}</strong>
                    </td>
                </tr>
                @else
                {{-- ── MONTHLY: standard rows ── --}}
                <tr><td class="lbl">Basic Salary</td><td class="ra ct">{{ $_fmtMoney($_storedSalary) }}</td></tr>
                <tr><td class="lbl">Per Day Rate</td><td class="ra val">{{ $_fmtMoney($perDaySalary) }}</td></tr>
                <tr><td class="lbl">Per Hour Rate</td><td class="ra val">{{ $_fmtMoney($perHourSalary) }}</td></tr>
                <tr>
                    ...
                    <td class="ra cb">
                        ...
                        {{ $daysPaidFmt }}
                        @if($paidLeaveDays > 0)
                            <span style="font-size:7.5px; color:#198754;">(+{{ $paidLeaveDays }} leave)</span>
                        @endif
                    </td>
                </tr>
                ...
                <tr><td class="lbl">Pay Period</td><td class="ra val" style="font-size:8.5px;">{{ $slipLabel }}</td></tr>
                <tr><td class="lbl">Pay Status</td><td class="ra val {{ $isPaid ? 'paid' : 'unpaid' }}">{{ $isPaid ? 'Paid' : 'Unpaid' }}</td></tr>
            </table>
        </td>
    </tr>
    </table>
    ...
    <tr>
        <td style="width:50%; padding:0; border:none; vertical-align:top;">
            ...
            <table class="bt">
                <tr><th style="text-align:left; padding-left:7px; width:65%;">Description</th><th class="ra">Amount (INR)</th></tr>
                @foreach($earRows as $er)
                <tr>
                    <td style="padding-left:7px;">{{ $er['label'] }}</td>
                    <td class="ra">@if($er['amount'] !== null){{ $_fmtMoney($er['amount']) }}@endif</td>
                </tr>
                @endforeach
                <tr class="tot"><td style="padding-left:7px;"><strong>Total Earnings</strong></td><td class="ra"><strong>{{ $_fmtMoney($totalEarnings) }}</strong></td></tr>
            </table>
        </td>
        <td style="width:50%; padding:0; border-left:1px solid #b2dee2; vertical-align:top;">
            ...
            <table class="bt">
                ...
                <tr>
                    ...
                    <td class="ra">
                        @if($dr2['amount'] !== null){{ $_fmtMoney($dr2['amount']) }}
                        @elseif(($dr2['label'] ?? '') !== '')&mdash;@endif
                    </td>
                </tr>
                @endforeach
                <tr class="tot"><td style="padding-left:7px;"><strong>Total Deductions</strong></td><td class="ra"><strong>{{ $_fmtMoney($totalDeductions) }}</strong></td></tr>
            </table>
        </td>
    </tr>
    </table>
    ...
    <tr>
        <td style="width:50%; text-align:center; padding:14px 8px; border:1px solid #b2dee2; vertical-align:middle;">
            @if($_stampUrl)
                <img src="{{ $_stampUrl }}" alt="Stamp" class="stamp-img">
            @else
                <div class="stamp">
                    {{ $companyName }}<br>
                    <span style="font-size:6px; display:block; margin-top:3px; letter-spacing:0.8px;">{{ strtoupper($slipLabel) }}</span>
                </div>
            @endif
