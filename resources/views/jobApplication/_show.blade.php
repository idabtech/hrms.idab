                            ...
                            <ul class="list-inline mb-0">
                                @can('Delete Job Application')
                                    <li class="list-inline-item">
                                        ...
                                        <a href="javascript:void(0)" data-confirm="Are You Sure?|This action can not be undone. Do you want to continue?" data-confirm-yes="document.getElementById('archive-form-{{$jobApplication->id}}').submit();">
                                    ...
                                    @if($jobApplication->is_archive==0)
                                        <li class="list-inline-item">
                                            <a href="javascript:void(0)" data-confirm="Are You Sure?|This action can not be undone. Do you want to continue?" data-confirm-yes="document.getElementById('delete-form-{{$jobApplication->id}}').submit();"><span class="badge badge-pill badge-soft-danger">{{__('Delete')}}</span></a>
                ...
                <div class="card-body">
                    ...
                    <div class="py-2 my-4 border-top ">
                        ...
                        <div class="col-12 text-right">
                            <a href="javascript:void(0)" data-url="{{route('job.on.board.create', $jobApplication->id)}}" data-ajax-popup="true" class="btn btn-xs btn-white btn-icon-only width-auto">  {{__('Add to Job OnBoard')}}</a>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col text-right">
                            @can('Create Interview Schedule')
                                <a href="javascript:void(0)" data-url="{{ route('interview-schedule.create',$jobApplication->id) }}" class="btn btn-xs btn-white btn-icon-only width-auto" data-ajax-popup="true" data-title="{{__('Create New Interview Schedule')}}">
                    ...
                    <div class="list-group list-group-flush mb-4">
                        @foreach($notes as $note)
                            <div class="list-group-item px-0">
                                <div class="row align-items-center">
                                    <div class="col">
                                        <a href="javascript:void(0)" class="d-block h6 text-sm mb-0">{{!empty($note->noteCreated)?$note->noteCreated->name:'-'}}</a>
                                        <p class="card-text text-sm text-muted mb-0">
                                            {{$note->note}}
                                        </p>
                                    </div>
                                    <div class="col-auto">
                                        <a href="javascript:void(0)" class=""> {{\Auth::user()->dateFormat($note->created_at)}}</a>
                                    </div>
                                    @can('Delete Job Application Note')
                                        @if($note->note_created==\Auth::user()->id)
                                            <div class="col-auto text-right">
                                                <a class="delete-icon" href="javascript:void(0)" data-confirm="Are You Sure?|This action can not be undone. Do you want to continue?" data-confirm-yes="document.getElementById('delete-form-{{$note->id}}').submit();"> <i class="fas fa-trash"></i></a>
