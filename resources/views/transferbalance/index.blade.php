                                                ...
                                                @can('Edit Transfer Balance')
                                                    <div class="action-btn bg-info ms-2">
                                                        <a href="javascript:void(0)" class="mx-3 btn btn-sm  align-items-center"
                                                            ...
                                                            title="" data-title="{{ __('Edit Transfer Balance') }}"
...
@push('script-page')
    <script>
        $(document).on('change', '[name="from_account_id"], [name="to_account_id"]', function() {
            let fromAccount = $('[name="from_account_id"]').val();
            let toAccount = $('[name="to_account_id"]').val();

            if (fromAccount && toAccount && fromAccount === toAccount) {
                show_toastr('{{ __('Error') }}', '{{ __('From and To Account cannot be the same.') }}',
                'error');
            }
        });
    </script>
