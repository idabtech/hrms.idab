...
<div class="modal-body">
    ...
    <div class="row mb-3">
        <div class="col-12">
            ...
            <form id="termination-attachment-form" enctype="multipart/form-data">
                @csrf
                <div class="input-group">
                    <input type="file" name="file" id="termination-attachment-file" class="form-control" multiple>
                    <button type="button" class="btn btn-primary" id="btn-upload-termination-attachment">
    ...
    <div id="termination-attachments-list">
                        ...
                        @foreach ($attachments as $attachment)
                            <tr id="attachment-row-{{ $attachment->id }}">
                                ...
                                <td>
                                    ...
                                    <a href="{{ route('termination.attachment.download', [$termination->id, $attachment->id]) }}" target="_blank" title="{{ $attachment->file_name }}">
                                ...
                                <td class="text-end">
                                    <a href="{{ route('termination.attachment.download', [$termination->id, $attachment->id]) }}" class="btn btn-sm btn-outline-primary me-1" data-bs-toggle="tooltip" title="{{ __('Download') }}">
...
<script>
(function() {
    var terminationId = {{ $termination->id }};
    var uploadUrl = '{{ route("termination.attachment.upload", $termination->id) }}';
    var attachmentsUrl = '{{ route("termination.attachments", $termination->id) }}';
    var csrfToken = '{{ csrf_token() }}';

    // Upload
    $('#btn-upload-termination-attachment').on('click', function() {
        var files = $('#termination-attachment-file')[0].files;
        if (files.length === 0) {
        ...
        function uploadNext(index) {
            if (index >= total) {
                ...
                $('#termination-attachment-file').val('');
    ...
    $(document).on('click', '.btn-delete-attachment', function() {
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: '{{ route("termination.attachment.delete", [$termination->id, ":aid"]) }}'.replace(':aid', attachmentId),
                    type: 'DELETE',
