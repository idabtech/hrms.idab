...
@push('script-page')
    ...
    <script>
        ...
        $(document).on('click', '#btn-hmrc-test-connection', function() {
            ...
            $btn.prop('disabled', true).html('<i class="fa fa-spinner fa-spin me-1"></i> {{ __("Testing...") }}');
            $result.hide();
            ...
            $.ajax({
                ...
                success: function(res) {
                    if (res.success) {
                        ...
                        $result.html('<span class="text-success fw-bold me-2"><i class="fa fa-check-circle text-success me-1"></i> ' + res.message + '</span>').show();
                    } else {
                        if (typeof show_toastr === 'function') {
                            show_toastr('Error', res.message, 'error');
                        }
                        $result.html('<span class="text-danger fw-bold me-2"><i class="fa fa-times-circle text-danger me-1"></i> ' + res.message + '</span>').show();
                    }
                },
                error: function(xhr) {
                    ...
                    $result.html('<span class="text-danger fw-bold me-2"><i class="fa fa-times-circle text-danger me-1"></i> ' + msg + '</span>').show();
                },
                complete: function() {
                    $btn.prop('disabled', false).html('<i class="fa fa-plug me-1"></i> {{ __("Test Connection") }}');
                }
            });
        });
    </script>
        ...
        <div class="col-sm-12">
            <div class="row">
                ...
                <div class="col-xl-9">
                    ...
                    <div id="pwa-settings" class="tab-pane">
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header">
                                    <h5>{{ __('PWA Settings') }}</h5>
                                    <small>{{ __('Configure your Progressive Web App (PWA) appearance and behavior.') }}</small>
                                </div>

                                {{ Form::open(['route' => 'pwa.settings.store', 'method' => 'post', 'enctype' => 'multipart/form-data']) }}
                                <div class="card-body p-4">
                                    <div class="row">
                                        <div class="col-lg-4 col-md-6 form-group">
                                            {{ Form::label('name', __('App Name'), ['class' => 'form-label']) }}
                                            {{ Form::text('name', $pwa['name'] ?? '', ['class' => 'form-control', 'placeholder' => __('Enter App Name')]) }}
                                        </div>

                                        <div class="col-lg-4 col-md-6 form-group">
                                            {{ Form::label('short_name', __('Short Name'), ['class' => 'form-label']) }}
                                            {{ Form::text('short_name', $pwa['short_name'] ?? '', ['class' => 'form-control', 'placeholder' => __('Enter Short Name')]) }}
                                        </div>

                                        <div class="col-lg-4 col-md-6 form-group">
                                            {{ Form::label('display', __('Display Mode'), ['class' => 'form-label']) }}
                                            {{ Form::select('display', ['fullscreen' => 'Fullscreen', 'standalone' => 'Standalone', 'minimal-ui' => 'Minimal UI', 'browser' => 'Browser'], $pwa['display'] ?? 'fullscreen', ['class' => 'form-control']) }}
                                        </div>

                                        <div class="col-lg-4 col-md-6 form-group">
                                            {{ Form::label('theme_color', __('Theme Color'), ['class' => 'form-label']) }}
                                            {{ Form::color('theme_color', $pwa['theme_color'] ?? '#6777ef', ['class' => 'form-control']) }}
                                        </div>

                                        <div class="col-lg-4 col-md-6 form-group">
                                            {{ Form::label('background_color', __('Background Color'), ['class' => 'form-label']) }}
                                            {{ Form::color('background_color', $pwa['background_color'] ?? '#6777ef', ['class' => 'form-control']) }}
                                        </div>

                                        <div class="col-lg-12 form-group">
                                            {{ Form::label('description', __('App Description'), ['class' => 'form-label']) }}
                                            {{ Form::textarea('description', $pwa['description'] ?? '', ['class' => 'form-control', 'rows' => 2, 'placeholder' => __('Enter App Description')]) }}
                                        </div>

                                        <div class="col-lg-6 form-group">
                                            {{ Form::label('logo', __('App Icon (512x512 PNG)'), ['class' => 'form-label']) }}
                                            <input type="file" name="logo" class="form-control"
                                                accept="image/png">
                                            @if (isset($pwa['icons'][0]['src']))
                                                <img src="{{ asset($pwa['icons'][0]['src']) }}" alt="App Icon"
                                                    height="64" class="mt-2">
                                            @endif
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div id="hmrc-settings">
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header">
                                    <h5>{{ __('HMRC API Settings') }}</h5>
                                    <small class="text-secondary font-weight-bold">
                                        {{ __('Configure your HMRC Developer Hub credentials for UK payroll integration (PAYE RTI, NINO Verification). Get these from') }}
                                        <a href="https://developer.service.hmrc.gov.uk" target="_blank">developer.service.hmrc.gov.uk</a>
                                    </small>
                                </div>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-12 form-group">
                                            <div class="d-flex align-items-center">
                                                ...
                                                <div class="form-check form-switch custom-switch-v1">
                                                    <input type="hidden" name="hmrc_enabled" value="off">
                                                    <input type="checkbox"
                                                        class="form-check-input input-primary"
                                                        id="hmrc_enabled"
                                                        name="hmrc_enabled"
