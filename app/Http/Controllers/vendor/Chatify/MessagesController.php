...
use App\Services\ChatifyMessengerExtended as Messenger;
...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        if ($request['type'] == 'user') {
            ...
            if ($fetch) {
                $userAvatar = app(\App\Services\ChatifyMessengerExtended::class)->getUserWithAvatar($fetch)->avatar;
            }
        }
    ...
    public function fetch(Request $request)
    {
        ...
        foreach ($messages->reverse() as $index => $message) {
            $allMessages .= Chatify::messageCard(
                Messenger::fetchMessage($message->id, $index)
            );
        }
    ...
    public function getContacts(Request $request)
    {
        ...
        if (count($usersList) > 0) {
            ...
            foreach ($usersList as $user) {
                $contacts_message = app(\App\Services\ChatifyMessengerExtended::class)->getContactItem($user);
    ...
    public function updateContactItem(Request $request)
    {
        ...
        $contactItem = app(\App\Services\ChatifyMessengerExtended::class)->getContactItem($user);
    ...
    public function search(Request $request)
    {
        ...
        foreach ($records->items() as $record) {
            $getRecords .= view('Chatify::layouts.listItem', [
                'get' => 'search_item',
                'type' => 'user',
                'user' => app(\App\Services\ChatifyMessengerExtended::class)->getUserWithAvatar($record),
            ])->render();
        }
        if ($records->total() < 1) {
