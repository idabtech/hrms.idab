...
use App\imports\MarkedAttendenceImport;
...
class AttendanceEmployeeController extends Controller
{
    ...
    public function attendance(Request $request)
    {
        ...
        if (empty($todayAttendance)) {
            ...
            // dd($totalLateSeconds,$hours, $mins, $secs ,' .......',$late);
    ...
    public function importAttendanceFile(Request $request)
    {
        return view('attendance.marked-import');
    }

    public function attendenceImport(Request $request){
        $rules = [
    ...
    //    $attendance = \Excel::import(new MarkedAttendenceImport, $request->file('file')->store('files'));
       if (($open = fopen($request->file, "r")) !== FALSE) 
        ...
        while (($data = fgetcsv($open, 1000, ",")) !== FALSE) {   
            if($data[0] != ''){
                if($data[0] != 'employee'){
                    // dd($data[0]);     
                    $employee = Employee::where(['name' => $data[0], 'created_by' => \Auth::user()->creatorId()])->first();
                    // dd($employee);
                    if(!empty($employee)){
                        $times = strtotime($data[1]);
                        $date = date('Y-m-d', $times);

                        $check_attendance = AttendanceEmployee::where(['employee_id'=>$employee->id, 'date' => $date]);
                        // dd($check_attendance);
                        // print_r($check_attendance); exit;
                        if(!isset($check_attendance->id)){
                            $attendance = new AttendanceEmployee();
                            $attendance->employee_id = $employee->id;
                            $attendance->date        = $date;
                            $attendance->status      = $data[2];
                            $attendance->clock_in    = $data[3];
                            $attendance->clock_out   = $data[4];
                            $attendance->late        = $data[5];
                            $attendance->early_leaving = $data[6];
                            $attendance->overtime    = $data[7];
                            $attendance->total_rest  = $data[8];
                            $attendance->created_by  = \Auth::user()->creatorId();
                            $attendance->save();
                        }
                        // dd(!isset($check_attendance->id));
                    }else{
                        return redirect()->back()->with('error','"'.$data[0].'" is Not Found in Employee list Please create');
                    }
                }
            }else{
                return redirect()->back()->with('success', 'Record imported successfuly');
            }
        }
