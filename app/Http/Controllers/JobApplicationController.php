...
use App\Mail\MoveJobApplication;
...
class JobApplicationController extends Controller
{
    ...
    public function create()
    {
        ...
        $job_category = JobCategory::where('created_by', \Auth::user()->creatorId())->get()->pluck('title', 'id');
        return view('jobApplication.create', compact('jobs', 'questions','branches','job_category'));
    }
    ...
    public function order(Request $request)
    {
        if(\Auth::user()->can('Move Job Application'))
        {
            ...
            $job  = JobApplication::where('id', '=', $post['order'])->first();
            ...
            $stages = JobStage::where(['created_by' => \Auth::user()->creatorId(),'id' => $job->stage])->first();

            // $data = array([
            //     'name'  => $job->name,
            // ]);
            // print_r($stages->title);exit;
            Mail::to($job->email)->send(new MoveJobApplication($job->name,$stages->title));
        }
        else
        {
            return redirect()->route('job-application.index')->with('error', __('Permission denied.'));
