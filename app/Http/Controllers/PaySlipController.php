...
class PaySlipController extends Controller
{
    ...
    public function index()
    {
        if (\Auth::user()->can('Manage Pay Slip') || \Auth::user()->type == 'employee') {
            ...
            $payslip = PaySlip::where('created_by', \Auth::user()->creatorId())->get()->pluck('name', 'id');
        //    dd($payslip);
            $month = [
                '01' => 'JAN',
                '02' => 'FEB',
            ...
            $allowances           = Allowance::where('employee_id', $employees->id)->get();
            $commissions          = Commission::where('employee_id', $employees->id)->get();
            $otherpayments        = OtherPayment::where('employee_id', $employees->id)->get();
            $bonous               = Bonous::where('employee_id', $employees->id)->get();
            $overtimes            = Overtime::where('employee_id', $employees->id)->get();
            $saturationdeductions = SaturationDeduction::where('employee_id', $employees->id)->get();
            $pension              = Pension::where('employee_id', $employees->id)->get();
            $loans                = Loan::where('employee_id', $employees->id)->get();
            $employee             = Employee::find($employees->id);

            }
            return view('payslip.index', compact('employees', 'month', 'year','department','payslip'));
        } else {
...
// print_r($employee->get_net_salary()); exit;
                $payslipEmployee                       = new PaySlip();
                $payslipEmployee->employee_id          = $employee->id;
                $payslipEmployee->net_payble           = $employee->get_net_salary();
                // dd($payslipEmployee->net_payble);
    ...
    public function getEmployeeByDepartment(Request $request){
        $payslip = PaySlip::where(['department' => $request['department'], 'created_by' => \Auth::user()->creatorId()])->get();
    ...
    public function search_json(Request $request)
    {
        ...
        if (empty($validatePaysilp)) {
            ...
            if($request->department > 0){
                $paylip_employee->where('employees.department_id',$request->department);
            }
            ...
            foreach ($paylip_employee as $employee) {
                ...
                if (Auth::user()->type == 'employee') {
                    if (Auth::user()->id == $employee->user_id) {
                        ...
                        $tmp[] = !empty($employee->net_payble) ? \Auth::user()->priceFormat($payslipDetail['net_salary']) : '-';
                        // print_r($payslipDetail['net_salary']); exit;
    ...
    public function pdf($id, $month)
    {
        ...
        // print_r($payslipDetail); exit;
        $net_salary = $employee->get_net_salary();

        return view('payslip.pdf', compact('payslip', 'employee', 'designation','payslipDetail'));
    }
