...
class ReferralProgramController extends Controller
{
    ...
    public function companyIndex()
    {
        ...
        $netAmount = $objUser->commission_amount - $paidAmount;
        $paymentRequest = TransactionOrder::where('status', 1)->where('req_user_id', $objUser->id)->first();
        return view('referral-program.company', compact('setting','netAmount', 'transactions', 'paidAmount', 'transactionsOrder', 'paymentRequest'));
    }
    ...
    {
        $referral = ReferralSetting::where('created_by', 1)->first();
        $minAmount = isset($referral) ? $referral->minimum_threshold_amount : 0;
        $paidAmount = TransactionOrder::where('req_user_id', $id)->where('status', 2)->sum('req_amount');
        $user = User::find($id);
        $netAmount = $user->commission_amount - $paidAmount;
     if($request->request_amount < $minAmount) {
            return redirect()->back()->with('error', __('This request cannot be send because it less than the threshold amount.'));
        }
        else
        {
            if($request->request_amount > $netAmount)
            {
                return redirect()->back()->with('error', __('This request cannot be send because it exceeds the commission amount.'));
            }
        }
