...
class InterviewScheduleController extends Controller
{
    ...
    public function create($candidate = 0)
    {
        ...
        $employees->prepend('--', '');
// $stages=JobApplication::where('created_by', \Auth::user()->creatorId())->get()->pluck('stage');
// $candidates = JobApplication::where('created_by', \Auth::user()->creatorId())
//     ->whereHas('jobStage', function ($query) {
//         $query->whereNotIn('title', ['Hired', 'Rejected', 'Interview','interview','rejected','hired']);
//     })->get()->pluck('name', 'id');

// $candidates = \DB::table('job_applications')
//     ->leftJoin('interview_schedules', 'job_applications.id', '=', 'interview_schedules.candidate')
//     ->get()->pluck('name', 'id');
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->can('Create Interview Schedule')) {
            ...
            $schedule->save();
            // $jobApplication = JobApplication::where('created_by', \Auth::user()->creatorId())->where('id', $request->candidate)->first();
            // $interview_stage=JobStage::where('created_by', \Auth::user()->creatorId())->where('title', 'Interview')->first()->id;
            //         $jobApplication->stage = $interview_stage;
            //         $jobApplication->save();
            // Google Celander
