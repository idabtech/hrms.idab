    ...
    public function destroy(DeductionOption $deductionoption)
    {
        if(\Auth::user()->can('Delete Deduction Option'))
        {
            if($deductionoption->created_by == \Auth::user()->creatorId())
            {
                ...
                {
                    return redirect()->route('deductionoption.index')->with('error', __('This Deduction Option has Statutory Deductions. Please remove the Statutory Deductions from this Deduction option.'));
                }
