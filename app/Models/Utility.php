...
class Utility extends Model
{
    ...
    public static function employeePayslipDetail($employeeId)
    {
        ...
        $deduction['leave']    = Leave::select('leaves.*','leave_types.title as leave_type')->leftjoin('leave_types','leave_types.id','=','leaves.leave_type_id')->where('leaves.employee_id', $employeeId)->where(function ($query) {
        ...
        $attendence = AttendanceEmployee::where('employee_id', $employeeId)->whereMonth('date',Carbon::now()->subMonth()->month)->count();
        ...
        if($total_working_days > $total_attendace){
            ...
            $deduction['leave'][] = (object)array(
                'leave_reason' => 'None clockin',
                'leave_type' => 'None clockin',
        ...
        $payslip['totalEarning']   = $totalAllowance + $totalCommission + $totalBonous + $earning['totalOverTime'] + $employess->salary;

        $payslip['deduction']      = $deduction;
        $payslip['totalDeduction'] = $totalloan + $totaldeduction + $totalPansion + $leaveAmount;

        $payslip['net_salary'] = $payslip['totalEarning'] - $payslip['totalDeduction'];

        // print_r($payslip['net_salary']); exit;
    ...
    //     $deduction['pansion']           = Loan::where('employee_id', $employeeId)->get();
