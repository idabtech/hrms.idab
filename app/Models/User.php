...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    public static function employeeIdFormat($number)
    {
        ...
        $department = Department::select('departments.slug')->leftjoin('employees','employees.department_id','=','departments.id')->where('employees.employee_id' , $number )->first();
        $slug = isset($department->slug) ? $department->slug : '';

    //    echo "<pre>"; print_r($department); exit;
        return $settings["employee_prefix"].$slug. sprintf("%05d", $number);
    }
