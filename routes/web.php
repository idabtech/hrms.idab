...
use App\Http\Controllers\OldDataManageController;
...
use App\Http\Controllers\PearkController;
...
// Handle SSO token passed as query param and redirect to attendance page
Route::get('/app', function (Request $request) {
    ...
    $personalAccessToken = PersonalAccessToken::findToken($tokenParam);
...
Route::get('openings-job', [JobController::class, 'globalCareer'])->name('global.career');
...
// temporery route for generate passcode of all accounts
// Route::get('passcode-generate', [AttendanceEmployeeController::class, 'generatePasscode']);
...
Route::group(['middleware' => ['verified']], function () {
    ...
    Route::group(
        ...
        function () {
            ...
            Route::any('event/export-event', [EventController::class, 'export_event'])->name('event.export-event');

            // SEO Settings
            Route::post('setting/seo-setting', [SettingsController::class, 'SeoSettings'])->name('seo.settings');

            // cache Settings
            Route::post('setting/cache-setting', [SettingsController::class, 'CacheSettings'])->name('clear.cache');
    ...
    Route::post('employee-sub', [EmployeeController::class, 'employeeSub'])->name('employee.sub')->middleware(
        [
            'auth',
            'XSS',
        ]
    ...
    Route::get('subdepartment', [SubDepartmentController::class, 'index'])->name('subdepartment')->middleware(
        [
            'auth',
            'XSS',
        ]
    ...
    Route::get('add/subdepartment', [SubDepartmentController::class, 'create'])->name('subdepartment.create')->middleware(
        [
            'auth',
            'XSS',
        ]
    );
    Route::post('add-subdepartment', [SubDepartmentController::class, 'store'])->name('subdepartment-create')->middleware(
        [
            'auth',
            'XSS',
        ]
    );
    Route::get('edit-subdepartment/{id}', [SubDepartmentController::class, 'edit'])->name('subdepartment-edit')->middleware(
        [
            'auth',
            'XSS',
        ]
    );
    Route::post('edit/subdepartment', [SubDepartmentController::class, 'update'])->name('subdepartment.edit')->middleware(
        [
            'auth',
            'XSS',
        ]
    );
    Route::get('delete-subdepartment/{id}', [SubDepartmentController::class, 'delete'])->name('subdepartment.delete')->middleware(
        [
            'auth',
            'XSS',
        ]
    ...
    Route::resource('employmenttype', EmploymentTypeController::class)->middleware(
        [
            'auth',
            'XSS',
        ]
    ...
    Route::get('employee/pension/{eid}', [PensionController::class, 'show'])->name('employee.pension.create');


    Route::resource('pension', PensionController::class)->middleware(
        [
            'auth',
            'XSS',
        ]
    ...
    Route::get('peark/create/{eid}', [PearkController::class, 'pearkcreate'])->name('employee.peark.create')->middleware(
        [
            'auth',
            'XSS',
        ]
    ...
    Route::resource('peark', PearkController::class)->middleware(
        [
            'auth',
            'XSS',
        ]
    ...
    Route::get('bonous/create/{eid}', [BonousController::class, 'bonouscreate'])->name('employee.bonous.create')->middleware(
        [
            'auth',
            'XSS',
        ]
    ...
    Route::resource('bonous', BonousController::class)->middleware(
        [
            'auth',
            'XSS',
        ]
    ...
    Route::post('employee/update/uk-payroll/{id}', [SetSalaryController::class, 'employeeUpdateUkPayroll'])->name('employee.uk.payroll.update')->middleware(
        [
            'auth',
            'XSS',
        ]
    ...
    Route::get('salary-revision/create/{employee_id}', [SalaryRevisionController::class, 'create'])
        ->name('salary-revision.create')
        ->middleware(['auth', 'XSS']);

    Route::post('salary-revision', [SalaryRevisionController::class, 'store'])
        ->name('salary-revision.store')
        ->middleware(['auth', 'XSS']);

    Route::get('salary-revision/{id}/edit', [SalaryRevisionController::class, 'edit'])
        ->name('salary-revision.edit')
        ->middleware(['auth', 'XSS']);

    Route::put('salary-revision/{id}', [SalaryRevisionController::class, 'update'])
        ->name('salary-revision.update')
        ->middleware(['auth', 'XSS']);

    Route::post('salary-revision/{id}/reject', [SalaryRevisionController::class, 'reject'])
        ->name('salary-revision.reject')
        ->middleware(['auth', 'XSS']);

    Route::delete('salary-revision/{id}', [SalaryRevisionController::class, 'destroy'])
        ->name('salary-revision.destroy')
        ->middleware(['auth', 'XSS']);

    Route::post('salary-revision/{id}/approve', [SalaryRevisionController::class, 'approve'])
    ...
    Route::get('payslip/uk-download/{id}/{m}', [PaySlipController::class, 'downloadUkPdf'])->name('payslip.uk.download')->middleware(
        [
            'auth',
            'XSS',
        ]
    ...
    Route::resource('attendanceemployee', AttendanceEmployeeController::class)->except(['update'])->middleware(
        [
            'auth',
            'XSS',
        ]
    ...
    Route::put('attendanceemployee/{attendanceemployee}', [AttendanceEmployeeController::class, 'update'])
        ->name('attendanceemployee.update')
        ->middleware(['auth', 'XSS', 'throttle:1,1']);
    ...
    // ── Rota page ────────────────────────────────────────────────────────────
    Route::get('rotas', [RotaController::class, 'index'])->name('rota.index')->middleware(['auth', 'XSS']);

    // ── Rota AJAX routes (session auth, CSRF protected) ───────────────────────
    Route::prefix('rota')->name('rota.')->middleware(['auth', 'XSS'])->group(function () {

        // Staff list          GET /rota/staff
        Route::get('staff',                [RotaController::class, 'apiStaff'])->name('staff');

        // Shifts daily summary GET /rota/shifts
        Route::get('shifts',               [RotaController::class, 'apiShifts'])->name('shifts');

        // Shift templates     GET /rota/shift-templates
        Route::get('shift-templates',      [RotaController::class, 'apiShiftTemplates'])->name('shift-templates');

        // Branches            GET /rota/branches
        Route::get('branches',             [RotaController::class, 'apiBranches'])->name('branches');

        // Assign/update single shift  POST /rota/assign-shift
        Route::post('assign-shift',        [RotaController::class, 'apiAssignShift'])->name('assign-shift');

        // Get single rota entry        GET /rota/rota-entry/{id}
        Route::get('rota-entry/{id}',      [RotaController::class, 'apiGetRotaEntry'])->name('rota-entry.get');

        // Delete single rota entry     DELETE /rota/rota-entry/{id}
        Route::delete('rota-entry/{id}',   [RotaController::class, 'apiDeleteRotaEntry'])->name('rota-entry.delete');

        // Bulk assign         POST /rota/bulk-assign
        Route::post('bulk-assign',         [RotaController::class, 'apiBulkAssign'])->name('bulk-assign');

        // Bulk remove         POST /rota/bulk-remove
        Route::post('bulk-remove',         [RotaController::class, 'apiBulkRemove'])->name('bulk-remove');
    ...
    Route::get('export/employee/file', [EmployeeController::class, 'exportFile'])->name('employee.file.export');
    Route::post('import/employee', [EmployeeController::class, 'import'])->name('employee.import');
    Route::get('export/employee/excelD', [EmployeeController::class, 'exportExcelD'])->name('employee.export');
    Route::get('export/employee/excelA', [EmployeeController::class, 'exportExcelA'])->name('employee.export.A');
    Route::get('export/employee/excelB', [EmployeeController::class, 'exportExcelB'])->name('employee.export.B');
    Route::get('export/employee/excelC', [EmployeeController::class, 'exportExcelC'])->name('employee.export.C');
    ...
    // Bulk routes MUST come before the resource and any wildcard {attendanceRequest} routes
    // so Laravel doesn't match "bulk-approve" as a model ID parameter.
    Route::post('attendance-requests/bulk-approve', [AttendanceRequestController::class, 'bulkApprove'])->middleware(['auth', 'XSS'])->name('attendance-requests.bulk-approve');
    Route::post('attendance-requests/bulk-decline', [AttendanceRequestController::class, 'bulkDecline'])->middleware(['auth', 'XSS'])->name('attendance-requests.bulk-decline');

    Route::resource('attendance-requests', AttendanceRequestController::class)->except(['store'])->middleware(['auth', 'XSS']);
    Route::put('attendance-requests/{attendanceemployee}/clock-out', [AttendanceRequestController::class, 'clockOut'])
        ->name('attendancerequests.clock_out')
        ->middleware(['auth', 'XSS', 'throttle:1,1']);
    Route::post('attendance-requests', [AttendanceRequestController::class, 'store'])->middleware(['auth', 'XSS', 'throttle:1,1'])->name('attendance-requests.store');
    Route::post('attendance-requests/{attendanceRequest}/approve', [AttendanceRequestController::class, 'approve'])->middleware(['auth', 'XSS'])->name('attendance-requests.approve');
    Route::post('attendance-requests/{attendanceRequest}/decline', [AttendanceRequestController::class, 'decline'])->middleware(['auth', 'XSS'])->name('attendance-requests.decline');
    ...
    Route::post('create-department', [CommonController::class, 'createDepartment'])->name('create-department');
    Route::post('create-branch', [CommonController::class, 'createBranch'])->name('create-branch');
    Route::post('create-designation', [CommonController::class, 'createDesignation'])->name('create-designation');
    Route::post('create-goal-type', [CommonController::class, 'creategoalType'])->name('create-goal-type');
    Route::post('create-training-type', [CommonController::class, 'createtrainingType'])->name('create-training-type');
    Route::post('create-trainer', [CommonController::class, 'createTrainer'])->name('create-trainer');
    Route::post('create-awardtype', [CommonController::class, 'createAwardType'])->name('create-awardtype');
    Route::post('create-designation', [CommonController::class, 'createDesignation'])->name('create-designation');
    Route::post('create-termination-type', [CommonController::class, 'createTerminationType'])->name('create-termination-type');
    Route::post('create-job-category', [CommonController::class, 'createJobCategory'])->name('create-job-category');
    Route::post('create-job', [CommonController::class, 'createJob'])->name('create-job');
    Route::post('create-allowance-options', [CommonController::class, 'createAllowanceOptions'])->name('create-allowance-options');
    Route::post('create-deduction-options', [CommonController::class, 'createDeductionOptions'])->name('create-deduction-options');
    Route::post('create-loan-options', [CommonController::class, 'createLoanOption'])->name('create-loan-options');
    Route::post('create-saturation-options', [CommonController::class, 'createSaturationOptions'])->name('create-saturation-options');
    Route::post('create-shift', [CommonController::class, 'createShift'])->name('create-shift');
    Route::post('create-employmenttype', [CommonController::class, 'createEmploymentType'])->name('create-employmenttype');
    Route::post('create-subDepartment', [CommonController::class, 'createSubDepartment'])->name('create-subDepartment');
    Route::post('create-income-category', [CommonController::class, 'createIncomeCategory'])->name('create-income-category');
    Route::post('create-payment-type', [CommonController::class, 'createPaymentMethod'])->name('create-payment-type');
    Route::post('create-expense-category', [CommonController::class, 'createExpenseCategory'])->name('create-expense-category');
    Route::get('get-branch', [CommonController::class, 'getBranch'])->name('get-branch');
    Route::get('get-department', [CommonController::class, 'getDepartment'])->name('get-department');
    Route::get('get-trainingtype', [CommonController::class, 'getTrainingType'])->name('get-trainingtype');
    Route::get('get-trainer', [CommonController::class, 'getTrainer'])->name('get-trainer');

    Route::get('wages-index', [WageController::class, 'index'])->name('wages.index')->middleware(
        [
            'auth',
            'XSS',
        ]
    );
    Route::get('wages-create', [WageController::class, 'create'])->name('wages.create')->middleware(
        [
            'auth',
            'XSS',
        ]
    );
    Route::post('wages-store', [WageController::class, 'save'])->name('wages-store')->middleware(
        [
            'auth',
            'XSS',
        ]
    );
    Route::get('edit-wages/{id}', [WageController::class, 'edit'])->name('edit-wages')->middleware(
        [
            'auth',
            'XSS',
        ]
    );
    Route::post('wages-update', [WageController::class, 'update'])->name('wages.update')->middleware(
        [
            'auth',
            'XSS',
        ]
    );
    Route::get('wages-delete/{id}', [WageController::class, 'delete'])->name('wages.delete')->middleware(
        [
            'auth',
            'XSS',
        ]
    ...
    Route::get('shift-turner', [ShiftTurnerController::class, 'index'])->name('shiftturner.index')->middleware(
        [
            'auth',
            'XSS',
        ]
    );
    Route::post('getsubdepartment', [ShiftTurnerController::class, 'getSubDepartmentandEmployee'])->name('getSubDepartmentandEmployee')->middleware(
        [
            'auth',
            'XSS',
        ]
    );
    Route::post('get-employee-by-subdepartment', [ShiftTurnerController::class, 'getEmployeeList'])->name('getEmployeeList')->middleware(
        [
            'auth',
            'XSS',
        ]
    );
    Route::post('save-shift-turner', [ShiftTurnerController::class, 'store'])->name('shiftturner.store')->middleware(
        [
            'auth',
            'XSS',
        ]
    ...
    Route::post('/employee/{employee}/update-shift', [ShiftTurnerController::class, 'updateShift'])->name('shiftturner.updateShift')->middleware(
        [
            'auth',
            'XSS',
        ]
    ...
    // Route::get('/auto-shift-rotate', [ShiftTurnerController::class, 'autoShiftRotation'])->name('shiftturner.auto-shift-rotate')->middleware(
    //     [
    //         'auth',
    //         'XSS',
    //     ]
