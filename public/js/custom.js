...
function formatTime12hours(time) {
    ...
    // Handle time range
    if (time.includes('-')) {
        const rangeParts = time.split(/\s*-\s*/);
        if (rangeParts.length === 2) {
            return `${formatSingleTime12hours(rangeParts[0])} - ${formatSingleTime12hours(rangeParts[1])}`;
        }
    }

    // Handle single time
    return formatSingleTime12hours(time);
}

function formatSingleTime12hours(time) {
    if (!time) {
        return '';
    }
